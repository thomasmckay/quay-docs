...
+
* **Registry Location**: Container repository you want to mirror from, including registry, namespace, and repository.
For example, registry.example.com/jsmith/myimage.
+
...
+
* **Robot User**: Select the robot account you created earlier to do the mirroring. A choice is presented here to create a new robot account.
+
...
Uncheck this box if, for example, you set up {productname} for testing against an external
registry with a self-signed certificate or no certificate.
+
* **HTTP Proxy, HTTPs Proxy, No Proxy**: Identify the proxy server needed to access the external registry, if one is required.
+
